...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const { sub, username, dir, role }: TokenPayload = await this.jwtService.verifyAsync(token, {
        secret: this.configService.get("JWT_SECRET"),
      });
      const user: _App.AuthUser = { id: sub, username, dir, role: role as _App.SystemRole };
      request.user = user;
      const now = Date.now();
