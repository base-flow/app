...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const { sub, username, nickname, roles, directory }: TokenPayload = await this.jwtService.verifyAsync(token, {
        secret: this.configService.get("JWT_SECRET"),
      });
      const user: _App.IAuthUser = { id: sub, username, nickname, directory, roles: roles as _Permission.SystemRole[] };
      request.user = user;
      const now = Date.now();
