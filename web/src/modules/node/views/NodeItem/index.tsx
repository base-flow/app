...
const Component: FC<Props> = ({ item, permissions, authId, setCurEdit, onDelete, onCollect, onItemClick }) => {
  if (isDirectory(item)) {
    return (
      <div className={classnames(styles.NodeItem, "g-card folder")} onClick={() => onItemClick(item)}>
        ...
        <div className="head-icon">
          <img className="icon" alt="node" src={DefaultFolderIcon} />
    ...
    return (
      <div className={classnames(styles.NodeItem, "g-card")} onClick={() => onItemClick(item)}>
        <Collect absolute id={item.id} value={undefined} onChange={onCollect} />
        <div className="head-icon">
          <img className="icon" alt="node" src={item.icon || DefaultIcon} />
          <h4 className="title">{item.name}</h4>
          <div className={classnames("info", `${styles.NodeItem}__package`)}>
