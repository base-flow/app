...
import { DeleteOutlined, EditOutlined, PlusCircleOutlined } from "@/components/Icons";
import LoadingMask from "@/components/LoadingMask";
import SearchInput from "@/components/SearchInput";
import Star from "@/components/Star";
import { FlagSrc } from "@/components/utils";
import { useEvent } from "@/hooks/tools";
import { FlowAppAPI } from "../../api";
import AppEdit from "../AppEdit";
import styles from "./index.module.scss";

const sortOptions = ["default", "updateDate"];
...
const AppList: FC<{ query: FlowApp.IQuery }> = ({ query }) => {
  ...
  const onSort = useEvent((sorterField?: string) => {
    router.navigate({ to: "/", search: { ...query, sorterField } });
  });
  ...
  return (
    <section className={`${styles.AppList} g-page`}>
      ...
      <div className="hd">
        <div>
          <Button className="g-big" color="primary" variant="text" icon={<PlusCircleOutlined />} onClick={onCreate}>
            创建应用
          </Button>
        </div>
        <div className="g-space">
          ...
          <div>
            <span>排序：</span>
            <FieldSorter value={query.sorterField} onChange={onSort} options={sortOptions} />
          </div>
        </div>
      </div>
      <div className="bd">
        <div className="g-grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5 2k:grid-cols-6">
          {appList.map((item) => {
            return (
              <Link className={`${styles.AppList}__card g-card ${item.type || ""}`} key={item.id} to="/apps/$appId/flows" params={{ appId: item.id }}>
